...
class CTC(object):
    ...
    def extend_target_with_blank(self, target):
        ...
        for i in range(1, N, 2):
            if i > 2 and extended_symbols[i-2] != extended_symbols[i]:
                skip_connect[i] = 1
    ...
    def get_forward_probs(self, logits, extended_symbols, skip_connect):
        ...
        for t in range(1, T):
            ...
            for i in range(1, S):
                ...
                if (i > 2 and extended_symbols[i] != extended_symbols[i-2]):
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            target_new = self.target[batch_itr][:self.target_lengths[batch_itr]]
            logits_new = self.logits[:self.input_lengths[batch_itr], batch_itr, :]
            extended_symbols, skip_connect = self.ctc.extend_target_with_blank(target_new)
            
            alpha = self.ctc.get_forward_probs(logits_new, extended_symbols, skip_connect)
            beta = self.ctc.get_backward_probs(logits_new, extended_symbols, skip_connect)
            ...
            for t in range(T):
                for i in range(R):
                    total_loss[batch_itr] -= gamma[t, i] * np.log(logits_new[t, extended_symbols[i]])
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            logits_new = self.logits[:self.input_lengths[batch_itr], batch_itr, :]
            # extended_symbols, skip_connect = self.ctc.extend_target_with_blank(target_new)
            # print(extended_symbols.shape, skip_connect)

            for t in range(self.input_lengths[batch_itr]):
                for r in range(len(self.extended_symbols[batch_itr])):
                    l = self.extended_symbols[batch_itr][r]
                    dY[t, batch_itr, l] -= self.gammas[batch_itr][t][r] / logits_new[t][l]
