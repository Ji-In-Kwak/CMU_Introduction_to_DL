## references
## https://github.com/hankyul2/EfficientNetV2-pytorch/blob/main/efficientnetv2/efficientnet_v2.py
...
class EfficientNetV2(nn.Module):
    ...
    def __init__(self, layer_infos, out_channels=1280, nclass=0, dropout=0.2, stochastic_depth=0.0,
        ...
        self.head_feat = nn.Sequential(OrderedDict([
            ('bottleneck', ConvBNAct(self.final_stage_channel, out_channels, 1, 1, 1, self.norm_layer, self.act)),
            ('avgpool', nn.AdaptiveAvgPool2d((1, 1))),
            ('flatten', nn.Flatten())
        ]))
    ...
    def forward(self, x, return_feats = False):
        if return_feats:
            return self.head_feat(self.blocks(self.stem(x)))
        else:
